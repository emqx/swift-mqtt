...
protocol StorageProtocol {
    ...
    func write(_ frame: Publish) -> Bool

    func write(_ frame: Pubrel) -> Bool

    func remove(_ frame: Publish)

    func remove(_ frame: Pubrel)
...
final class Storage: StorageProtocol {
    ...
    init?(by clientId: String) {
        guard let userDefault = UserDefaults(suiteName: Storage.name(clientId)) else {
            return nil
        }
    ...
    func remove(_ frame: Frame) {
        if let pub = frame as? Publish {
            userDefault.removeObject(forKey: key(pub.msgid))
        } else if let rel = frame as? Pubrel {
    ...
    private func __read(needDelete: Bool)  -> [Frame] {
        ...
        for (k, v) in allObjs {
            ...
            if let f = Publish(packetFixedHeaderType: parsed.0, bytes: parsed.1) {
                frames.append(f)
            } else if let f = Pubrel(packetFixedHeaderType: parsed.0, bytes: parsed.1) {
                frames.append(f)
            }
        }
        return frames
