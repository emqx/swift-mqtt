...
protocol MQTTPacket: CustomStringConvertible, Sendable {
    ...
    static func read(version: MQTT.Version, from: MQTTIncomingPacket) throws -> Self
}
...
extension MQTTPacket {
    ...
    func writeFixedHeader(packetType: MQTTPacketType, flags: UInt8 = 0, size: Int, to byteBuffer: inout DataBuffer) {
        byteBuffer.writeInteger(packetType.rawValue | flags)
        MQTTSerializer.writeVariableLengthInteger(size, to: &byteBuffer)
    }
}

extension MQTT.Version {
...
struct MQTTConnectPacket: MQTTPacket {
    ...
    func write(version: MQTT.Version, to byteBuffer: inout DataBuffer) throws {
        ...
        // payload
        try MQTTSerializer.writeString(self.clientId, to: &byteBuffer)
        if let will {
            if version == .v5_0 {
                try will.properties.write(to: &byteBuffer)
            }
            try MQTTSerializer.writeString(will.topicName, to: &byteBuffer)
            try MQTTSerializer.writeData(will.payload, to: &byteBuffer)
        }
        if let username {
...
struct MQTTPublishPacket: MQTTPacket {
    ...
    func write(version: MQTT.Version, to byteBuffer: inout DataBuffer) throws {
        ...
        // write payload
        byteBuffer.writeData(self.publish.payload)
    }

    static func read(version: MQTT.Version, from packet: MQTTIncomingPacket) throws -> Self {
    ...
    func packetSize(version: MQTT.Version) -> Int {
        ...
        // payload
        size += self.publish.payload.count
        return size
    }
}
...
struct MQTTPubAckPacket: MQTTPacket {
    var description: String { "\(self.type)(id:\(packetId),reason:\(reason))" }
...
struct MQTTSubAckPacket: MQTTPacket {
    var description: String { "\(self.type)(id:\(packetId),reason:\(reasons))" }
    let type: MQTTPacketType
    let packetId: UInt16
    let reasons: [MQTTReasonCode]
    let properties: Properties

    init(type: MQTTPacketType, packetId: UInt16, reasons: [MQTTReasonCode], properties: Properties = .init()) {
        self.type = type
        self.packetId = packetId
...
struct MQTTIncomingPacket: MQTTPacket {
    ...
    /// everything
//    static func read(from byteBuffer: inout DataBuffer) throws -> MQTTIncomingPacket {
...
//        return MQTTIncomingPacket(type: type, flags: byte & 0xF, remainingData: buffer)
//    }
}
