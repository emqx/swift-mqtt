...
public struct PublishProperties {
    ...
    public var properties: [UInt8] {
        ...
        if let payloadFormatIndicator = self.payloadFormatIndicator {
            properties += getMQTTPropertyData(type: Property.payloadFormatIndicator.rawValue, value: [payloadFormatIndicator.rawValue])
        }
        //3.3.2.3.3  Message Expiry Interval
        if let messageExpiryInterval = self.messageExpiryInterval {
            properties += getMQTTPropertyData(type: Property.willExpiryInterval.rawValue, value: messageExpiryInterval.byteArrayLittleEndian)
        }
        //3.3.2.3.4 Topic Alias
        if let topicAlias = self.topicAlias {
            properties += getMQTTPropertyData(type: Property.topicAlias.rawValue, value: topicAlias.hlBytes)
        }
        //3.3.2.3.5 Response Topic
        if let responseTopic = self.responseTopic {
            properties += getMQTTPropertyData(type: Property.responseTopic.rawValue, value: responseTopic.bytesWithLength)
        }
        //3.3.2.3.6 Correlation Data
        if let correlationData = self.correlationData {
            properties += getMQTTPropertyData(type: Property.correlationData.rawValue, value: correlationData)
        }
