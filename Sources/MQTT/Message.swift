...
extension MQTT{
    public class Message: NSObject {
        ...
        public var string: String? {
            String(data: .init(bytes: payload, count: payload.count), encoding: .utf8)
        }
        public var properties: [UInt8] {
            ...
            if let willDelayInterval = self.willDelayInterval {
                properties += getMQTTPropertyData(type: Property.willDelayInterval.rawValue, value: willDelayInterval.byteArrayLittleEndian)
            }
            /// 3.1.3.2.4 Message Expiry Interval
            if let willExpiryInterval = self.willExpiryInterval {
                properties += getMQTTPropertyData(type: Property.willExpiryInterval.rawValue, value: willExpiryInterval.byteArrayLittleEndian)
            }
            /// 3.1.3.2.3 Payload Format Indicator
            ...
            if var contentType = self.contentType {
                ...
                properties += getMQTTPropertyData(type: Property.contentType.rawValue, value: contentType.bytesWithLength)
            }
            /// 3.1.3.2.6 Response Topic
            if var willResponseTopic = self.willResponseTopic {
                ...
                properties += getMQTTPropertyData(type: Property.responseTopic.rawValue, value: willResponseTopic.bytesWithLength)
            }
            /// 3.1.3.2.7 Correlation Data
            if let willCorrelationData = self.willCorrelationData {
                let buff = UInt16(willCorrelationData.count).hlBytes + willCorrelationData
                properties += getMQTTPropertyData(type: Property.correlationData.rawValue, value: buff)
            }
            /// 3.1.3.2.8 User Property
            if let willUserProperty = self.willUserProperty {
                willUserProperty.forEach { element in
                    properties.append(UInt8(Property.userProperty.rawValue))
                    if isUTF8EncodedData {
        ...
        public init(topic: String, payload: [String: Any], qos: MQTTQos = .qos1, retained: Bool = false) throws {
...
extension MQTT.Message {
    
    public override var description: String {
        return "MQTT(topic: \(topic), qos: \(qos), payload: \(payload.summary))"
    }
}
...
extension MQTT.Message {
    ...
    var t_pub_frame: Publish {
        var frame = Publish(topic: topic, payload: payload, qos: qos, msgid: 0)
