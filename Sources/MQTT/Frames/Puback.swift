...
struct Puback: Frame {
    ...
    public var pubAckProperties: DecodePubAck?
    ...
    ///MQTT 5.0
    init(msgid: UInt16, reasonCode: PubAckReasonCode) {
        self.msgid = msgid
...
extension Puback: InitialWithBytes {
    ...
    init?(packetFixedHeaderType: UInt8, bytes: [UInt8]) {
        ...
        if bytes.count > 2{
            self.reasonCode = PubAckReasonCode(rawValue: bytes[2])
            self.pubAckProperties = DecodePubAck()
...
extension Puback: CustomStringConvertible {
