...
struct Suback: Frame {
    ...
    public var subAckProperties: DecodeSubAck?
...
extension Suback: InitialWithBytes {
    ...
    init?(packetFixedHeaderType: UInt8, bytes: [UInt8]) {
        ...
        if (protocolVersion == "5.0"){
            ...
            self.reasonCodes = [SubAckReasonCode]()
            for i in 3 ..< bytes.count {
                guard let qos = SubAckReasonCode(rawValue: bytes[i]) else {
                    return nil
                }
                self.reasonCodes! += [qos]
            }

            self.subAckProperties = DecodeSubAck()
...
extension Suback: CustomStringConvertible {
